...
def _batch_tokenize(captions: list[str], text_length: int = 77) -> torch.Tensor:
    tokenizer = SimpleTokenizer()
    tokens = [
        tokenize(caption, tokenizer=tokenizer, text_length=text_length, truncate=True)
...
def _collate_texts(batch: list[dict], text_length: int) -> dict:
    ...
    return {
        "text_tokens": _batch_tokenize(captions, text_length=text_length),
        "indices": torch.tensor([item["index"] for item in batch], dtype=torch.long),
    }


def _collate_reid_texts(batch: list[dict], text_length: int) -> dict:
    captions = [item["caption"] for item in batch]
    return {
        "text_tokens": _batch_tokenize(captions, text_length=text_length),
        "person_ids": torch.tensor([item["person_id"] for item in batch], dtype=torch.long),
...
def encode_reid_images(
    ...
    *,
    ...
    dataset = _ReIDImageDataset(items, transform)
    loader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
...
def encode_reid_texts(
    ...
    *,
    ...
    loader = DataLoader(
        ...
        pin_memory=torch.cuda.is_available(),
        persistent_workers=num_workers > 0,
        collate_fn=lambda batch: _collate_reid_texts(batch, text_length),
    )
    features: list[torch.Tensor] = []
    person_ids: list[torch.Tensor] = []
...
def _infer_num_classes(train_args) -> int:
    dataset = __factory[train_args.dataset_name](root=Path(train_args.root_dir))
    return len(dataset.train_id_container)


def _load_irra_model(
    ...
    train_config_path = (
        Path(config.train_config) if config.train_config.strip() else checkpoint_dir / "configs.yaml"
    )
    ...
    train_args = load_train_configs(str(train_config_path))
    train_args.training = False
    train_args.output_dir = str(checkpoint_dir)
    train_args.root_dir = str(dataset_root)
    num_classes = _infer_num_classes(train_args)
    model = build_model(train_args, num_classes=num_classes)
    Checkpointer(model).load(f=str(checkpoint_path))
    model.to(device)
    model.eval()
    transform = build_transforms(img_size=train_args.img_size, is_train=False)
    text_length = int(getattr(train_args, "text_length", 77))
...
def run_sugarcrepe(config_path: Path) -> None:
    ...
    model, transform, text_length = _load_irra_model(config, device, dataset_root)
    ...
    output = {
        ...
        "checkpoint": str(Path(config.checkpoint_dir) / "best.pth"),
        "split": config.test_split,
