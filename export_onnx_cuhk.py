...
def export_onnx_cuhk():
    config_path = "/home/jayn2u/IRRA/irra_cuhk/configs.yaml"
    model_path = "/home/jayn2u/IRRA/irra_cuhk/best.pth"
    ...
    dummy_image = torch.randn(batch_size, 3, 256, 256, dtype=torch.float16).to(device)
    dummy_text = torch.randint(0, 49408, (batch_size, 77), dtype=torch.long).to(device)

    output_dir = "/home/jayn2u/IRRA/irra_cuhk"
    ...
    img_onnx_path = os.path.join(output_dir, "irra_cuhk_image_encoder.onnx")
    try:
        torch.onnx.export(
            img_enc,
            (dummy_image,),
    ...
    txt_onnx_path = os.path.join(output_dir, "irra_cuhk_text_encoder.onnx")
    try:
        torch.onnx.export(
            txt_enc,
            (dummy_text,),
            ...
            dynamic_axes={'text': {0: 'batch_size'}, 'text_embeddings': {0: 'batch_size'}}
        )
